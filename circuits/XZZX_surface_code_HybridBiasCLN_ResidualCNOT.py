# This script constructs the circuits for the XZZX code with a fully tailored 
# noise model that considers bias preserving CZ gates and CNOT gates that may
# present some remainder bias.
import stim
...
class CircuitGenParametersXZZXBiased():
    """
    Parameters for generating a rotated XZZX surface code circuit using an hybrid biased noise model with CNOTs having remainder bias.
    
    The noise model is defined in the parameters:
    p: overall error probability
    η: bias parameter
    η2: remainder CNOT bias
    ...
    Two Qubit Errors (CNOT gates, ZX basis, remainder bias):
        p2= pix2 + piy2 + piz2 + pxi2 + pxx2 + pxy2 + pxz2 + pyi2 + pyx2 + pyy2 + pyz2 + pzi2 + pzx2 + pzy2 + pzz2
        and η2 = (piz2 + pzi2 + pzz2) / (pix2 + piy2 + pxi2 + pxx2 + pxy2 + pxz2 + pyi2 + pyx2 + pyy2 + pyz2 + pzx2 + pzy2)
        
        therefore:
        piz2 = pzi2 = pzz2 = η2*p / [3*(1+η2)]
        pix2 = piy2 = pxi2 = pxx2 = pxy2 = pxz2 = pyi2 = pyx2 = pyy2 = pyz2 = pzx2 = pzy2 = p2 / [12*(1+η2)]
    ...
    - after_clifford_depolarization:
        This parameter will be used for the depolarizing errors (X basis operations) by Hadamard gates.

    - afterCNOT_bias_probability:
        Each CNOT gate is followed by a two-qubit Pauli channel, for which pzz2, pzi2 and piz2 can occur with probability η2*p / [3*(1+η2)] each,
        and the remaining 12 non-trivial two-qubit Paulis can each occur with probability p2 / [12*(1+η2)]. This is done to consider the remainder bias
        introduced by CNOTs in two-level qubits.
    """
    ...
    afterCNOT_bias_probability: tuple = (0,0) # remainder bias errors afte CNOT gates
...
def create_rotated_XZZX_surface_code_circuit_biased_noise(params: CircuitGenParametersXZZXBiased,
    ...
    # 1. Apply first layer of CNOT  gates
    if params.afterCNOT_bias_probability[0] > 0:
        p = params.afterCNOT_bias_probability[0]
        eta = params.afterCNOT_bias_probability[1]
    ...
    # 4. apply fourth layer of CNOT gates
    if params.afterCNOT_bias_probability[0] > 0:
        p = params.afterCNOT_bias_probability[0]
        eta = params.afterCNOT_bias_probability[1]
        bias_probabilities = [p/(12*(1+eta))]*15

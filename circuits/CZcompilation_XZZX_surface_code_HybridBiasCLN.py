# This script constructs the circuits for the XZZX code with a compilation
# only using CZ gates. Those are considered bias preserving here.
import stim
...
class CircuitGenParametersXZZXBiased():
    """
    Parameters for generating a rotated XZZX surface code with only CZ gates using an hybrid biased noise model.
    ...
    - after_clifford_depolarization:
        ...
        involves Hadamards where this depolarizing errors will arise. This is the compilation only using CZ
    """
    
    rounds: int
    distance: int = None
    after_clifford_depolarization: float = 0 # this refers to the depolarizing errors of the Hadamard
...
def create_rotated_XZZX_surface_code_circuit_biased_noise(params: CircuitGenParametersXZZXBiased,
    ...
    # We will have CNOTs,CZs,CZs,CNOTs (XZZX) then measure checks and reset those
    ...
    # the two qubit gates for extracting the XZZX stabilizers are applied in 6 steps (including the necessary Hadamard gate layers)
    # 1. Apply first layer of CZs
    cycle_actions.append("TICK", [])
    if params.afterCZ_bias_probability[0] > 0:
        p = params.afterCZ_bias_probability[0]
        eta = params.afterCZ_bias_probability[1]
        # pzz, pzi and piz can occur with probability η*p / [3*(1+η)] each,
        # and the remaining 12 non-trivial two-qubit Paulis can each occur with probability p / [12*(1+η)].
        # order of the probabilities: pix piy piz  pxi pxx pxy pxz  pyi yx yy yz  zi zx zy zz
    ...
    cycle_actions.append("CZ", cnot_targets[0])
    if params.afterCZ_bias_probability[0] > 0:
        cycle_actions.append("PAULI_CHANNEL_2", cnot_targets[0], bias_probabilities)
    ...
    # 2. Apply Hadamard gates to the data qubits (this is a test XZZX compiled with CZ only compilation)
    cycle_actions.append("TICK", [])     
    cycle_actions.append("H", data_qubits)
    if params.after_clifford_depolarization > 0:
        cycle_actions.append("DEPOLARIZE1", measurement_qubits, params.after_clifford_depolarization)
    # Biased channel on the data qubits as per idling in this time step
    if params.before_round_data_bias_probability[0] > 0:
        ...
        p_y = p/(2*(1+eta))
        p_z = p*eta / (1+eta)
        cycle_actions.append("PAULI_CHANNEL_1", measurement_qubits, [p_x, p_y, p_z])

    # 3. & 4. Apply second layers of CZ gates
    cycle_actions.append("TICK", [])
    if params.afterCZ_bias_probability[0] > 0:
        p = params.afterCZ_bias_probability[0]
        eta = params.afterCZ_bias_probability[1]
        ...
        bias_probabilities[-4] = eta*p / (3*(1+eta))
        bias_probabilities[-1] = eta*p / (3*(1+eta))
    cycle_actions.append("CZ", cnot_targets[1])
    if params.afterCZ_bias_probability[0] > 0:
        cycle_actions.append("PAULI_CHANNEL_2", cnot_targets[1], bias_probabilities)
    # Idling errors to unused checks
    if params.before_round_data_bias_probability[0] > 0:
        p = params.before_round_data_bias_probability[0]
        eta = params.before_round_data_bias_probability[1]
        ...
        p_y = p/(2*(1+eta))
        p_z = p*eta / (1+eta)
        cycle_actions.append("PAULI_CHANNEL_1", list((set(data_qubits) | set(measurement_qubits)) - (set( cnot_targets[1]))), [p_x, p_y, p_z]) 
    
    # 4. Apply fourth layer of CZ gates
    cycle_actions.append("TICK", [])
    if params.afterCZ_bias_probability[0] > 0:
        p = params.afterCZ_bias_probability[0]
        eta = params.afterCZ_bias_probability[1]
        ...
        bias_probabilities[-4] = eta*p / (3*(1+eta))
        bias_probabilities[-1] = eta*p / (3*(1+eta))
    cycle_actions.append("CZ", cnot_targets[2])
    if params.afterCZ_bias_probability[0] > 0:
        cycle_actions.append("PAULI_CHANNEL_2", cnot_targets[2], bias_probabilities)
    # Idling errors to unused checks
    if params.before_round_data_bias_probability[0] > 0:
        p = params.before_round_data_bias_probability[0]
        eta = params.before_round_data_bias_probability[1]
        ...
        p_y = p/(2*(1+eta))
        p_z = p*eta / (1+eta)
        cycle_actions.append("PAULI_CHANNEL_1", list((set(data_qubits) | set(measurement_qubits)) - (set( cnot_targets[2]))), [p_x, p_y, p_z]) 
    
    # 5. Apply Hadamard gates to the data qubits (this is a test XZZX compiled with CZ only compilation)
    cycle_actions.append("TICK", [])     
    cycle_actions.append("H", data_qubits)
    if params.after_clifford_depolarization > 0:
        cycle_actions.append("DEPOLARIZE1", measurement_qubits, params.after_clifford_depolarization)
    ...
    # 6. Apply last layer of CZ gates
    cycle_actions.append("TICK", [])
    if params.afterCZ_bias_probability[0] > 0:
        p = params.afterCZ_bias_probability[0]
        eta = params.afterCZ_bias_probability[1]
        ...
        bias_probabilities[-4] = eta*p / (3*(1+eta))
        bias_probabilities[-1] = eta*p / (3*(1+eta))
    cycle_actions.append("CZ", cnot_targets[3])
    if params.afterCZ_bias_probability[0] > 0:
        cycle_actions.append("PAULI_CHANNEL_2", cnot_targets[3], bias_probabilities)
    ...
    # In order to make the XZZX code work for the CZ only compilation it's changed how to initialize them
    # the codes should be invariant though, only difference is the extraction ordering and the pattern initialization.
    # Note that here the "data_qubits_z" aer initialized in X basis and the "data_qubits_x" in the Z. This is why
    # then the vertical and horizontal memories are switched for the results of this compilation.
    head.append("RX", data_qubits_z)
    head.append("R", data_qubits_x)
    if params.after_reset_flip_probability > 0:
        append_anti_basis_error_biased(head, data_qubits_z, params.after_reset_flip_probability, "X")
        append_anti_basis_error_biased(head, data_qubits_x, params.after_reset_flip_probability, "Z")
    ...
    if params.afterCZ_bias_probability[0] > 0:
        ...
        bias_probabilities[-4] = eta*p / (3*(1+eta))
        bias_probabilities[-1] = eta*p / (3*(1+eta))
    head.append("CZ", cnot_targets[0])
    if params.afterCZ_bias_probability[0] > 0:
        head.append("PAULI_CHANNEL_2", cnot_targets[0], bias_probabilities)
    # Idling errors to unused checks
    if params.before_round_data_bias_probability[0] > 0:
        p = params.before_round_data_bias_probability[0]
        eta = params.before_round_data_bias_probability[1]
    ...
    # 2. Apply Hadamard gates to the data qubits (this is a test XZZX compiled with the CZ only compilation)
    head.append("TICK", [])    
    head.append("H", data_qubits)
    if params.after_clifford_depolarization > 0:
        head.append("DEPOLARIZE1", measurement_qubits, params.after_clifford_depolarization)
    # Biased channel on the data qubits as per idling in this time step
    if params.before_round_data_bias_probability[0] > 0:
        ...
        p_y = p/(2*(1+eta))
        p_z = p*eta / (1+eta)
        head.append("PAULI_CHANNEL_1", measurement_qubits, [p_x, p_y, p_z])

    # 3. Apply second layer of CZ gates
    if params.afterCZ_bias_probability[0] > 0:
        p = params.afterCZ_bias_probability[0]
        eta = params.afterCZ_bias_probability[1]
        bias_probabilities = [p/(12*(1+eta))]*15
    ...
    head.append("TICK", [])
    head.append("CZ", cnot_targets[1])
    if params.afterCZ_bias_probability[0] > 0:
        head.append("PAULI_CHANNEL_2", cnot_targets[1], bias_probabilities)
    # Idling errors to unused checks
    if params.before_round_data_bias_probability[0] > 0:
        p = params.before_round_data_bias_probability[0]
        eta = params.before_round_data_bias_probability[1]
        ...
        p_y = p/(2*(1+eta))
        p_z = p*eta / (1+eta)
        head.append("PAULI_CHANNEL_1", list((set(data_qubits) | set(measurement_qubits)) - (set( cnot_targets[1]))), [p_x, p_y, p_z]) 
    
    # 4. Apply third layer of CZ gates
    head.append("TICK", [])
    if params.afterCZ_bias_probability[0] > 0:
        p = params.afterCZ_bias_probability[0]
        eta = params.afterCZ_bias_probability[1]
        ...
        bias_probabilities[-4] = eta*p / (3*(1+eta))
        bias_probabilities[-1] = eta*p / (3*(1+eta))
    head.append("CZ", cnot_targets[2])
    if params.afterCZ_bias_probability[0] > 0:
        head.append("PAULI_CHANNEL_2", cnot_targets[2], bias_probabilities)
    # Idling errors to unused checks
    if params.before_round_data_bias_probability[0] > 0:
        p = params.before_round_data_bias_probability[0]
        eta = params.before_round_data_bias_probability[1]
        ...
        p_y = p/(2*(1+eta))
        p_z = p*eta / (1+eta)
        head.append("PAULI_CHANNEL_1", list((set(data_qubits) | set(measurement_qubits)) - (set( cnot_targets[2]))), [p_x, p_y, p_z]) 
    ...
    # 6. Apply fourth layer of CZ gates
    head.append("TICK", [])
    if params.afterCZ_bias_probability[0] > 0:
        p = params.afterCZ_bias_probability[0]
        eta = params.afterCZ_bias_probability[1]
        ...
        bias_probabilities[-4] = eta*p / (3*(1+eta))
        bias_probabilities[-1] = eta*p / (3*(1+eta))
    head.append("CZ", cnot_targets[3])
    if params.afterCZ_bias_probability[0] > 0:
        head.append("PAULI_CHANNEL_2", cnot_targets[3], bias_probabilities)
    ...
    # Reset the check qubits (all are in the X basis), first round different, the checks are initialized in parallel with the datas
    tail.append("TICK", [])
    ...
    # the two qubit gates for extracting the XZZX stabilizers with CZ compilation are applied in 6 steps
    # 1. Apply first layer of CZs
    tail.append("TICK", [])
    if params.afterCZ_bias_probability[0] > 0:
        p = params.afterCZ_bias_probability[0]
        ...
        bias_probabilities[-4] = eta*p / (3*(1+eta))
        bias_probabilities[-1] = eta*p / (3*(1+eta))
    tail.append("CZ", cnot_targets[0])
    if params.afterCZ_bias_probability[0] > 0:
        tail.append("PAULI_CHANNEL_2", cnot_targets[0], bias_probabilities)
    # Idling errors to unused checks
    if params.before_round_data_bias_probability[0] > 0:
        p = params.before_round_data_bias_probability[0]
        eta = params.before_round_data_bias_probability[1]
    ...
    if params.before_round_data_bias_probability[0] > 0:
        ...
        p_y = p/(2*(1+eta))
        p_z = p*eta / (1+eta)
        tail.append("PAULI_CHANNEL_1", measurement_qubits, [p_x, p_y, p_z])
    ...
    if params.afterCZ_bias_probability[0] > 0:
        ...
        bias_probabilities[-1] = eta*p / (3*(1+eta))
    tail.append("TICK", [])
    tail.append("CZ", cnot_targets[1])
    if params.afterCZ_bias_probability[0] > 0:
        tail.append("PAULI_CHANNEL_2", cnot_targets[1], bias_probabilities)
    # Idling errors to unused checks
    if params.before_round_data_bias_probability[0] > 0:
        p = params.before_round_data_bias_probability[0]
        eta = params.before_round_data_bias_probability[1]
        ...
        p_y = p/(2*(1+eta))
        p_z = p*eta / (1+eta)
        tail.append("PAULI_CHANNEL_1", list((set(data_qubits) | set(measurement_qubits)) - (set( cnot_targets[1]))), [p_x, p_y, p_z]) 
    ...
    if params.afterCZ_bias_probability[0] > 0:
        ...
        bias_probabilities[-4] = eta*p / (3*(1+eta))
        bias_probabilities[-1] = eta*p / (3*(1+eta))
    tail.append("CZ", cnot_targets[2])
    if params.afterCZ_bias_probability[0] > 0:
        tail.append("PAULI_CHANNEL_2", cnot_targets[2], bias_probabilities)
    # Idling errors to unused checks
    if params.before_round_data_bias_probability[0] > 0:
        p = params.before_round_data_bias_probability[0]
        eta = params.before_round_data_bias_probability[1]
        ...
        p_y = p/(2*(1+eta))
        p_z = p*eta / (1+eta)
        tail.append("PAULI_CHANNEL_1", list((set(data_qubits) | set(measurement_qubits)) - (set( cnot_targets[2]))), [p_x, p_y, p_z]) 

    # 5. Apply Hadamard gates to the data qubits (this is a test XZZX compiled with the CZ only compilation)
    tail.append("TICK", [])     
    tail.append("H", data_qubits)
    if params.after_clifford_depolarization > 0:
        tail.append("DEPOLARIZE1", measurement_qubits, params.after_clifford_depolarization)
    ...
    if params.afterCZ_bias_probability[0] > 0:
        ...
        bias_probabilities[-4] = eta*p / (3*(1+eta))
        bias_probabilities[-1] = eta*p / (3*(1+eta))
    tail.append("CZ", cnot_targets[3])
    if params.afterCZ_bias_probability[0] > 0:
        tail.append("PAULI_CHANNEL_2", cnot_targets[3], bias_probabilities)
    ...
    # then the vertical and horizontal memories are switched for the results of this compilation.
    if params.before_measure_flip_probability > 0:
        append_anti_basis_error_biased(tail, data_qubits_z, params.before_measure_flip_probability, "X")
        append_anti_basis_error_biased(tail, data_qubits_x, params.before_measure_flip_probability, "Z")  
    for q in data_qubits:
